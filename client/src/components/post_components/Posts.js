...
class Posts extends Component{
  ...
  render(){
    ...
    const placeholder = (this.props.wall==='public') ? 'Share your thoughts' : 'Write on your friend\'s wall';
    ...
    (
      <div onClick={this.emphasizeForm.bind(this)} className="panel clearfix panel-default comment-form">
            <form className="" action="index.html" method="post">
              <textarea  placeholder={placeholder} ref="comment" name="comment" rows="8" cols="20"></textarea>
            </form>
              <div onClick={submitFunction} className="btn btn-primary">Post</div>
        </div>
    );

    let posts = (this.props.posts.length>0) ?
    this.props.posts.sort(function(left, right){
        if(right.time>left.time) return 1;
        if(left.time>right.time) return -1;
        return 0;
    })
    : [];
    console.log('posts sorted by time: ',posts);
